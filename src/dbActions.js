...
function todoCreateNew(
    ...
    if (projectRetrieve("getAll").length == 0) {
        return;
    } //prevent creation of todo item if no project list available.

    const allTodosForProject = todoRetrieve(projectId, true);
    const lastPriorityNumber =
        allTodosForProject.length == 0
            ? 0
            : allTodosForProject[allTodosForProject.length - 1].priority;

    const newTodoItem = new itemTodo(
        projectId,
        title,
        description,
        dueDate,
...
function todoRetrieve(id, getAll = false) {
    ...
    if (getAll) {
        const allTodosForProject = todoDB.filter((element) => {
            return element.projectId == id;
        });
        return allTodosForProject;
    }
    const filteredItem = todoDB.filter((element) => {
...
function projectRetrieve(id) {
    ...
    const filteredItem = projectDB.filter((element) => {
        return element.id == id;
    });
    return filteredItem[0];
}
